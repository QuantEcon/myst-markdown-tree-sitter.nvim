...
### Changed
- **Suppressed verbose debug logging by default** - Plugin no longer shows initialization messages on startup
  - Added `debug = false` configuration option to control debug logging
  - Set `debug = true` in setup() to see verbose messages for troubleshooting

## [0.4.3] - 2025-12-04

### Fixed
- **Fixed `:MystDebug` parser detection for myst filetype** - Parser check now correctly requests the `markdown` parser for `myst` buffers instead of looking for a non-existent `myst` parser
